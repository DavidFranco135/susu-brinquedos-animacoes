...
interface RentalsProps {
  ...
  toys: Toy[];
  categories: string[]; // Recebe as categorias do Inventory via App.tsx
}
...
const Rentals: React.FC<RentalsProps> = ({ rentals, setRentals, customers, setCustomers, toys, categories }) => {
  ...
  // Estado para o filtro de categoria dentro do modal de nova reserva
  ...
  // Lista de categorias formatada para o select
  const modalCategories = useMemo(() => {
    const baseCats = categories && categories.length > 0 ? categories : ['Geral'];
    return ['Todas', ...baseCats];
  }, [categories]);

  // Filtra os brinquedos baseados na categoria selecionada no modal
  ...
  // Cálculo automático do valor total (Brinquedos + Valor Adicional)
  useEffect(() => {
    if (!toys) return;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setRentals((prev: any) => 
      editingRental 
        ? prev.map((r: any) => r.id === editingRental.id ? newRental : r) 
        : [...prev, newRental]
    );
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">Sincronizado com o Catálogo de Brinquedos.</p>
        </div>
        <button onClick={() => handleOpenModal()} className="bg-blue-600 text-white px-8 py-4 rounded-3xl font-black text-sm uppercase tracking-widest shadow-xl hover:scale-105 transition-all flex items-center gap-2">
          ...
          <tbody className="divide-y divide-slate-50">
            {filteredRentals.map(rental => (
              <tr key={rental.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-8 py-6 font-bold text-blue-600">R$ {rental.totalValue.toLocaleString('pt-BR')}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[150] flex items-center justify-center p-4">
          <form onSubmit={handleSubmit} className="bg-white w-full max-w-2xl rounded-[40px] p-10 shadow-2xl space-y-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="md:col-span-2 space-y-4">
                <div className="flex items-center justify-between">
                  <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Brinquedos</label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-48 overflow-y-auto p-4 bg-slate-50 rounded-[32px] border-2 border-slate-100">
                  {filteredToysForModal.map(t => (
                    <label key={t.id} className={`flex items-center gap-3 p-4 rounded-2xl border transition-all cursor-pointer ${formData.toyIds?.includes(t.id) ? 'bg-white border-blue-500' : 'bg-white/50 border-transparent'}`}>
                      <input type="checkbox" className="w-5 h-5 rounded-lg text-blue-600" checked={formData.toyIds?.includes(t.id)} onChange={e => {
              ...
              {/* Valor Adicional (Frete/Extras) */}
              <div className="space-y-2">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Valor Adicional (Frete/Extras)</label>
            ...
            <div className="bg-slate-900 p-8 rounded-[40px] flex items-center justify-between text-white shadow-xl">
                 <div>
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[3px] mb-1">Total Geral</p>
                    <h3 className="text-3xl font-black">R$ {(formData.totalValue || 0).toLocaleString('pt-BR')}</h3>
                 </div>
                 <div className="text-right">
                    <p className="text-blue-400 text-[10px] font-black uppercase tracking-[2px] mb-1">Restante a Pagar</p>
                    <h3 className="text-xl font-black">R$ {((formData.totalValue || 0) - (formData.entryValue || 0)).toLocaleString('pt-BR')}</h3>
                 </div>
            </div>

            <button type="submit" className="w-full py-5 bg-blue-600 text-white font-black text-sm uppercase tracking-widest rounded-3xl shadow-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-2">
              <CheckCircle2 size={18}/> Salvar Reserva
            </button>
          </form>
        </div>
      )}
