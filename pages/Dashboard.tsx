...
interface DashboardProps {
  rentals: Rental[];
  toysCount: number;
  transactions: FinancialTransaction[];import React, { useMemo } from 'react';
import { useNavigate } from 'react-router-dom';
import {
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell
} from 'recharts';
import {
  CalendarDays, Plus, Clock, Star, DollarSign
} from 'lucide-react';
import { Rental, RentalStatus, FinancialTransaction } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ rentals, toysCount, transactions }) => {
  ...
  const stats = useMemo(() => {
    ...
    const incomeMonth =
      rentals
        .filter(r => r.date.startsWith(currentMonth) && r.status !== RentalStatus.CANCELLED)
        .reduce(
          (acc, r) => acc + (r.status === RentalStatus.COMPLETED ? r.totalValue : r.entryValue),
          0
        ) +
      transactions
  ...
  const chartData = useMemo(() => {
    ...
    return months.map((name, index) => {
      ...
      const income =
        rentals
          .filter(r => r.date.startsWith(monthStr) && r.status !== RentalStatus.CANCELLED)
          .reduce(
            (acc, r) => acc + (r.status === RentalStatus.COMPLETED ? r.totalValue : r.entryValue),
            0
          ) +
      ...
      return { name, valor: income - expense };
    });
  ...
  const StatCard: React.FC<StatCardProps> = ({
    title, value, icon, color, subValue, path
  }) => {
    ...
    return (
      <div
        ...
        className="bg-white p-7 rounded-[32px] border border-slate-100 shadow-sm flex items-start justify-between cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all"
      >
        <div>
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-800">Painel Executivo</h1>
          <p className="text-slate-500">Visão geral {currentYear}</p>
        </div>
        <button
          onClick={() => navigate('/reservas')}
          className="flex items-center gap-3 bg-blue-600 text-white px-8 py-4 rounded-3xl font-black shadow-xl hover:scale-105 transition"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        <StatCard
          title="Reservas / Mês"
          value={stats.eventsMonth.length}
      ...
      <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
        <h2 className="text-xl font-black mb-6">Desempenho {currentYear}</h2>
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData}>
              ...
              <Bar dataKey="valor" radius={[10, 10, 0, 0]}>
                {chartData.map((d, i) => (
                  <Cell key={i} fill={d.valor >= 0 ? '#3B82F6' : '#EF4444'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
