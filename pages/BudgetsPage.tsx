        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[400] flex items-center justify-center p-4 print:p-0">
          <div className="bg-white w-full max-w-4xl max-h-[95vh] print:max-h-none rounded-[50px] print:rounded-none overflow-hidden flex flex-col print:block shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto print:overflow-visible">
              <div className="p-8 bg-white text-slate-800" id="budget-print-area" style={{ width: '100%', maxWidth: '900px', margin: '0 auto' }}>
                 <div className="flex flex-col md:flex-row justify-between items-start border-b-2 border-slate-900 pb-6 gap-6">
                    <div className="space-y-3">
                      <div className="w-20 h-20 rounded-[32px] overflow-hidden border-2 border-slate-900 mb-2">
                      ...
                      <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tighter leading-none">Proposta de<br/>Locação</h1>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-6">
                    <div className="p-4 bg-slate-50 rounded-[40px] border border-slate-100">
                      <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Cliente Solicitante</p>
                      <p className="font-black text-xl text-slate-900 mb-1">{selectedRental.customerName}</p>
                      <p className="text-sm text-slate-500 font-medium">Orçamento personalizado para seu evento.</p>
                    </div>
                    <div className="p-4 bg-blue-50/50 rounded-[40px] border border-blue-100/50">
                      <p className="text-xs font-black text-blue-400 uppercase tracking-widest mb-2">Data Estimada</p>
                      <p className="font-black text-xl text-slate-900 mb-1">{new Date(selectedRental.date + 'T00:00:00').toLocaleDateString('pt-BR')}</p>
                 ...
                 <div className="space-y-4">
                    <p className="text-xs font-black text-slate-400 uppercase border-b pb-2 tracking-widest">Especificação de Itens</p>
                    <table className="w-full text-left">
                      <thead>
                        <tr className="text-xs font-black text-slate-300 uppercase tracking-widest">
                          <th className="py-2">Descrição Atração</th>
                      ...
                      <tbody className="divide-y divide-slate-100 text-base">
                        {selectedRental.toyIds.map(tid => {
                          ...
                          return (
                            <tr key={tid}>
                              <td className="py-3">
                                <p className="font-black text-slate-900">{toy?.name} {toy?.size ? `(${toy.size})` : ''}</p>
                                {toy?.description && <p className="text-sm text-slate-500 mt-1">{toy.description}</p>}
                              </td>
                              <td className="py-3 text-right text-slate-900 font-black">R$ {(toy?.price || 0).toLocaleString('pt-BR')}</td>
                            </tr>
                          );
                        })}
                      </tbody>
                 ...
                 <div className="mt-8 bg-slate-900 p-6 rounded-[50px] flex flex-col md:flex-row justify-between items-center text-white gap-6">
                    <div>
                      <p className="text-xs font-black text-slate-500 uppercase tracking-[4px] mb-1">Total da Proposta</p>
                      <p className="text-4xl font-black">R$ {(selectedRental.totalValue || 0).toLocaleString('pt-BR')}</p>
                    </div>
                    <div className="bg-white/5 p-4 rounded-[40px] border border-white/10 text-center md:text-left min-w-[280px]">
                      <p className="text-xs font-black uppercase tracking-[2px] text-slate-400 mb-1">Formas de Pagamento</p>
